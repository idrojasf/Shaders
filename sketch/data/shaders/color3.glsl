...
vec3 setTritanopia(vec3 color){
	vec3 newColor;
	newColor.x = color.x;
	newColor.y = color.y;
	newColor.z = 0.494207*color.x + 1.24827*color.y;

	return newColor;
}


vec3 correctTritanopia(vec3 lms, vec3 deut){
...
void main(void)
{
	...
	if(tritanopia){
		color = setTritanopia(lms);
		color = correctTritanopia(lms, color);
	}
